...
local theme = {
	Foo = { bg = c.neutral_00, fg = c.neutral_09 },

	Comment = { fg = c.neutral_05 }, -- any comment
	--ColorColumn = { bg = c.black }, -- used for the columns set with 'colorcolumn'
	--Conceal = { fg = c.dark5 }, -- placeholder characters substituted for concealed text (see 'conceallevel')
	Cursor = { fg = c.neutral_09, bg = c.neutral_00 }, -- character under the cursor
	...
	LineNr = { fg = c.neutral_02 }, -- Line number for ":number" and ":#" commands, and when 'number' or 'relativenumber' option is set.
	...
	Constant = { fg = c.purple_dark }, -- (preferred) any constant
...
local function get_group(name)
	...
	while name do
		if fallbacks[name] then
			return (lang or "") .. fallbacks[name]
		end
		name, lang = name:match("(.*)%.(.*)")
	end
end
...
local function load()
	...
	if is_new_style() then
		for group, hl in pairs(treesitter_defaults) do
			if not theme[group] then
				highlight(group, hl)
			end
		end
	end
